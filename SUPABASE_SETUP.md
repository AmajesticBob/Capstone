...
### Setting Up API with Google Cloud Run

- These actions will done within the Capstone_API repo, not the frontend.

### Step 1: Install the `gcloud` CLI, Create Project

Because there are different methods to install the CLI, depending on your OS, simply search up the steps online. The end result should be the same either way.

- After Installation, run the command `gcloud init` in your command line, and follow the instructions. Always selecting the default option is perfectly fine. The final result should be a project, either a completely new one, or one that has already been used. In the case 

Now to access the project directly, in your browser, go to Google Cloud Console, then press "Go to my Console". This should lead to a Welcome page, and this page will be used to review the status of the API.

### Step 2: Enable APIs

In Google Cloud Console (GCC), click the top left, to open up the side panel (Navigation Menu). Head down to "APIs & Services". Clicking it should automatically take you to the "Enabled APIs & services" tab. Near the top, should be a button that says " + Enable APIs & services ". Click on it. From there, APIs can be found and enable with ease.
...
⚠️ ***Note***: For future reference, "Project ID" can be found in GCC, at the top left. Click on the button with your project's name, which should open the "Select a Project" pop-up. Your project should be listed there, along with it's "ID". 

If authentication hasn't been done in the command line at this point, the commands `gcloud auth login` and then `cloud config set project YOUR_PROJECT_ID` could be run. Otherwise disregard.

### Step 3: Build in the Cloud

- **Run this command**

This will build the `Dockerfile` in the cloud, and automatically push it to the Artifact Registry, and create the 'fashionclip-api-repo' repository as well.

```
gcloud builds submit \
  --tag us-central1-docker.pkg.dev/YOUR_PROJECT_ID/fashionclip-api-repo/fashionclip-api:latest
```
Note: This process will take a conisiderable amount of time to complete, less then 10 minutes, but that is normal

### Step 4: Create Secrets

The Supabase Key and URL used in the frontend will also be used here, by adding them to the project as "Secrets"

1. **Go to GCC -> Security -> Secret Manager**

2. **Click "Create Secret", Call it `SUPABASE_URL`, Paste your Supabase URL as the Secret value**

3. **One more time, click "Create Secret", Call it `SUPABASE_KEY`, Paste your Supabase Key as the Secret value**

4. **Back in the Secret Manager page, select either `SUPABASE_KEY` or `SUPABASE_URL`, and in the "Permissions" tab, click the "Add Principal"  button**

5. **not done with this yet**

### Step 5: Deploy the Build

- **Run this command**

After the build was successfully submitted, deploy the API
```
gcloud run deploy fashionclip-api \
  --image us-central1-docker.pkg.dev/YOUR_PROJECT_ID/fashionclip-api-repo/fashionclip-api:latest \
  --platform managed \
  --region us-central1 \
  --allow-unauthenticated \
  --memory 4Gi
```
Note: This process will take a conisiderable amount of time to complete, about 10 minutes, but that is normal

### Step 6: Use The API

- After the deploy command finishes, it will give you a Service URL like: `https://fashionclip-api-xxxxxxxx-uc.a.run.app`.

- Once again in GCC, open the left side panel (Navigation Menu) -> Cloud Run -> Overview
- If deployment was indeed successful, the API will appear in the "Most Used Resources" section
- Click on it, and it will give more details over the API. Close to the top should appear "URL: https://..." which holds the URL endpoint. In the frontend, in


⚠️ **Warning**: PythonAPI Url needs to be changed, in items.ts