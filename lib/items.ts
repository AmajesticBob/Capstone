 ...
 */
export async function searchSimilarItems(query: {
  queryText?: string;
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Error from FashionCLIP API search:', results.error);
      throw new Error(results.error || 'Failed to search for items');
    }
